...
int find_first_fit(struct header *free_list_ptr, uint64_t size) {
  ...
  while (free_list_ptr) {
    ...
    else if (free_list_ptr->size == size) {
    }
...
int find_worst_fit(struct header *free_list_ptr, uint64_t size) {
  ...
  while (free_list_ptr) {
    if (free_list_ptr->size > worst_fit_size) {
...
/** Part 2
 * (I assumed the address increased from left to right.)
 * ptr = head;
 * while (ptr != NULL) {
 *   if (ptr->next == head) {
 *     ptr->next = NULL;
 ...
 *   }
 *
 *   if (ptr->left_block == new) {
 *     ptr->size += new->size;
 *     new->next = ptr->next;
 *     ptr = new;
 *   }
 *   if (ptr->right_block == new) {
 *     ptr->size += new->size;
 *     new = ptr;
 *   }
 *   ptr = ptr->next;
