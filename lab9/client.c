...
2. Is it UDP or TCP? How do you know?
   : TCP. It uses SOCK_STREAM for the socket type.
3. The client is going to send some data to the server. Where does it get this data from? How can you tell in the code?
   : It gets the data from command line input. Line 54 reads from STDIN_FILENO.
4. How does the client program end? How can you tell that in the code?
   : The client ends if it receives an empty input. Line 54 implies num_read must be greater than 1
   to perform write().
*/
